...
use plugin\xbDevDoc\api\DocApi;
...
return [
    ...
    'apidoc' => [
        ...
        'responses' => [
            // 成功响应体
            'success' => [
                ['name' => 'status', 'desc' => '业务代码', 'type' => 'int', 'require' => 1],
                ['name' => 'msg', 'desc' => '业务消息', 'type' => 'string', 'require' => 1],
                //参数同上 headers；main=true来指定接口Returned参数挂载节点
                ['name' => 'data', 'desc' => '业务数据', 'main' => true, 'type' => 'object|array', 'require' => 1],
            ],
            // 异常响应体
            'error' => [
                ['name' => 'status', 'desc' => '业务代码', 'type' => 'int', 'require' => 1, 'md' => '/docs/HttpError.md'],
                ['name' => 'msg', 'desc' => '业务消息', 'type' => 'string', 'require' => 1],
            ],
        ],
        // （选配）全局响应状态码
        ...
        'ignored_annitation' => [],
        // （选配）解析时忽略的方法
        'ignored_methods' => ['__construct', '__call', '__callStatic'],
        // （选配）模型查询函数
        'model_query_function' => function ($sql) {
            return \support\think\Db::query($sql);
        },
        // （选配）数据库配置
        'database' => [],
        // （选配）Markdown文档
            ...
            'actions' => [
                [
                    ...
                    'click' => function ($shareData, $apiData) {
                        ...
                        $path = base_path() . '/runtime/apidoc.json';
                        $jsonFile = fopen(public_path() . $path, "w") or die("无法打开接口下载文件");
