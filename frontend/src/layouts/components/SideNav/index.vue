...
/** 站点名称（带 fallback） */
const siteName = computed(() => siteConfigStore.config?.system?.web_name || '镜影漫创')
/** 站点 Logo（带 fallback） */
...
<template>
  ...
  >
    ...
    <div class="flex flex-col items-center py-3 border-b border-border-subtle gap-1">
      <img :src="siteLogo" :alt="siteName" class="w-9 h-9 rounded-lg object-cover" />
      <span v-if="!collapsed" class="text-[10px] font-bold text-content whitespace-nowrap">
        {{ siteName }}
      </span>
    </div>
