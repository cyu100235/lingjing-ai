...
interface Props {
  rechargePackages: RechargeAmountItem[]
  selectedPackage: RechargeAmountItem | null
...
const emit = defineEmits<{
  selectPackage: [pkg: RechargeAmountItem]
  selectPayMethod: [method: 'alipay' | 'wechat']
  goToQrCode: []
}>()

function selectPackage(pkg: RechargeAmountItem) {
...
<template>
  <div class="px-6 py-5">
    ...
    <div class="mb-5">
      ...
      <div class="grid grid-cols-3 gap-3 max-h-64 overflow-y-auto pr-1 pt-2">
        <button v-for="pkg in rechargePackages" :key="pkg.recharge_amount"
          class="relative flex flex-col items-center p-3 rounded-xl border-2 transition-all duration-200"
          :class="selectedPackage?.recharge_amount === pkg.recharge_amount
            ? 'border-yellow-400 bg-yellow-400/5 shadow-[0_0_12px_rgba(250,204,21,0.15)]'
            : 'border-border hover:border-yellow-400/40'"
          @click="selectPackage(pkg)">
          <span v-if="Number(pkg.give_amount) > 0"
            class="absolute -top-2 right-1 px-1.5 py-0.5 rounded-full text-[10px] font-medium bg-red-500 text-white whitespace-nowrap">
            赠 {{ pkg.give_amount }}
          </span>
          <XbIcon name="coins" :size="20" class="text-yellow-400 mb-1.5" />
          <span class="text-sm font-bold"
            :class="selectedPackage?.recharge_amount === pkg.recharge_amount ? 'text-yellow-400' : 'text-content'">
            {{ pkg.recharge_amount }}
            {{ currencyName }}
          </span>
          <span class="text-base font-bold mt-0.5"
            :class="selectedPackage?.recharge_amount === pkg.recharge_amount ? 'text-yellow-400' : 'text-content'">
            <span class="text-xs">¥</span>{{ pkg.recharge_amount }}
          </span>
        </button>
      </div>
    </div>
    ...
    <button
      ...
      确认支付 ¥{{ selectedPackage?.recharge_amount }}
    </button>
