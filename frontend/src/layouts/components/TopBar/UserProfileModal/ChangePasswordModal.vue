...
async function handleSubmit() {
  ...
  try {
    const { success, message } = await userStore.changePasswordAction({
      origin_password: form.oldPassword,
      password: form.newPassword
    })
    if (success) {
      step.value = 'success'
    } else {
      XbMessage.error(message || '密码修改失败')
    }
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : '密码修改失败，请稍后重试'
    XbMessage.error(msg)
  } finally {
