...
/** 充值是否已关闭 */
const rechargeDisabled = computed(() => siteConfigStore.config?.user?.recharge?.recharge_state === '10')
  ...
  <header class="h-14 flex items-center justify-between px-6 border-b border-border-subtle bg-surface-elevated/50">
    ...
    <div class="flex items-center gap-3">
      ...
      <button
        ...
        @click="handleRechargeClick" :title="siteConfigStore.currencyName + '充值'">
        <XbIcon name="coins" :size="18" class="text-yellow-400" />
        <span class="text-xs font-medium text-yellow-400 hidden sm:inline">
          {{ siteConfigStore.currencyBalance }}
        </span>
        <span
          class="w-4 h-4 rounded-full bg-yellow-400/15 flex items-center justify-center group-hover:bg-yellow-400/25 transition-colors">
      ...
      <div
        ...
        <template v-if="userStore.isLoggedIn">
          <img :src="userStore.userInfo.avatar || '/images/user-avatar.png'" alt="用户头像"
            class="w-7 h-7 rounded-full object-cover shrink-0" />
        </template>
        <template v-else>
          <div
            class="w-7 h-7 rounded-full bg-surface-overlay border border-border flex items-center justify-center shrink-0">
