...
<template>
  <div class="grid grid-cols-5 gap-3">
    <div v-for="asset in assets" :key="asset.id" class="card-base overflow-hidden cursor-pointer group relative"
      :class="{ 'ring-2 ring-brand': batchMode && selectedIds.includes(asset.id) }" @click="$emit('cardClick', asset)">
      ...
      <div class="p-2.5">
        <h4 class="text-xs font-medium text-content truncate">
          {{ asset.name }}
        </h4>
        <div class="flex items-center justify-between mt-1.5">
          ...
          <div class="flex items-center gap-1">
            <span v-for="tag in asset.tags.slice(0, 2)" :key="tag"
