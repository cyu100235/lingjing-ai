    ...
    <template v-if="listing && !success">
      ...
      <div class="p-4 rounded-lg bg-surface-overlay mb-4">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-1">
            ...
            <span class="text-xs text-content-tertiary">金币</span>
          </div>
        </div>
        <div class="flex items-center justify-between mt-2 pt-2 border-t border-border-subtle">
          ...
          <span class="text-sm" :class="store.wallet.balance >= listing.price ? 'text-content' : 'text-danger'">
            {{ store.wallet.balance }} 金币
          </span>
        </div>
        <div class="flex items-center justify-between mt-1">
          <span class="text-xs text-content-tertiary">支付后余额</span>
          <span class="text-sm" :class="store.wallet.balance >= listing.price ? 'text-content' : 'text-danger'">
            {{ store.wallet.balance - listing.price }} 金币
          </span>
        </div>
      </div>
      ...
      <div class="flex justify-end gap-2">
        ...
        <button
          ...
          :disabled="store.wallet.balance < listing.price"
          @click="emit('confirm')"
        >
          <XbIcon name="shopping-cart" :size="14" />
          {{ store.wallet.balance < listing.price ? '余额不足' : '确认购买' }}
        </button>
      </div>
    </template>
