...
function createWorks(payload: { title: string; cover: string; aspectRatio: AspectRatio }) {
  ...
  router.push(`/works/episodes?work_id=${works.id}`)
}
...
function openEpisodes(id: string) {
  router.push(`/works/episodes?work_id=${id}`)
}
...
function handleCreateFromCard(type: 'script' | 'storyboard' | 'novel') {
  if (type === 'novel') {
    showComingSoonModal.value = true
    return
  }
...
<template>
  <div class="animate-fade-in">
    <!-- Creation Cards -->
    <CreationCards @create="handleCreateFromCard" />
