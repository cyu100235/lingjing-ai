...
import { createModalId, pushModal, removeModal, isTopModal } from '../modalStack'
...
// Keyboard support — only respond if this is the topmost modal
function handleKeydown(e: KeyboardEvent) {
  if (!isTopModal(modalId)) return
  // Determine whether the event originated from an editable element
  ...
  if (e.key === 'Escape' && !props.persistent) {
    e.preventDefault()
    handleClose()
  } else if ((e.key === 'Enter' || e.key === ' ') && props.submitOnEnter && !isEditable) {
    e.preventDefault()
    emit('submit')
  }
}
...
watch(() => props.visible, (val) => {
  if (val) {
    ...
    activeAnimation.value = props.animation
      || (props.randomAnimation
        ? ALL_ANIMATIONS[Math.floor(Math.random() * ALL_ANIMATIONS.length)]
        : 'scale')
    document.addEventListener('keydown', handleKeydown)
    document.body.style.overflow = 'hidden'
    // Move focus to modal content so background form inputs lose focus
    nextTick(() => {
  ...
  <Teleport to="body">
    <Transition name="xb-modal">
      <div v-if="visible" class="xb-modal-overlay" @click.self="handleOverlayClick">
        <div ref="modalContentRef" class="xb-modal-content relative" tabindex="-1" :class="[width, noPadding ? 'xb-modal-content--no-padding' : '', `xb-modal-anim-${activeAnimation}`]">
          <!-- Close button -->
          <button
            v-if="showClose"
          ...
          <!-- Footer -->
          <div v-if="$slots.footer" class="mt-6 flex-shrink-0">
            <slot name="footer" />
          </div>
        </div>
      </div>
...
.xb-modal-content:focus-visible {
  outline: none;
  box-shadow: none;
}
