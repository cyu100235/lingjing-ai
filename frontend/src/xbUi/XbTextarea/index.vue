  ...
  <div class="xb-textarea-wrapper">
    ...
    <div class="relative">
      ...
      <div
        ...
        :class="{ 'right-8': clearable && modelValue && !disabled && !readonly }"
      >
        <slot name="top-right" />
      </div>
      ...
      <textarea
        ...
        :class="[
          textareaClass,
          resizeClass,
          ($slots['bottom-left'] || $slots['bottom-right'] || maxlength) ? 'pb-8' : '',
        ]"
        @input="handleInput"
        @focus="handleFocus"
        @blur="handleBlur"
      ...
      <!-- Bottom bar -->
      <div
        v-if="$slots['bottom-left'] || $slots['bottom-right'] || maxlength"
        class="absolute left-2 right-2 bottom-2 z-10 flex items-center justify-between"
      >
        <div class="flex items-center gap-2">
          <slot v-if="$slots['bottom-left']" name="bottom-left" />
          <span v-if="maxlength" class="text-xs select-none" :class="countClass">
            {{ modelValue.length }}/{{ maxlength }}
          </span>
        </div>
        <div v-if="$slots['bottom-right']">
